...
class ColdOutreachGenerator {
    ...
    try {
        ...
        // Em caso de erro, retornar null para indicar falha na análise real
        return null;
    }
}
    ...
    estimateFollowers(doc) {
        ...
        return 0; // Retorna 0 se não conseguir estimar
    }
...
async makeRealRequest(url, retries = 2) {
    ...
    analyzeStore(storeName, websiteData) {
        ...
        if (websiteData) {
            // Extrair oportunidades e armazenar
            const opportunities = analysis.oportunidade.split(" | ").filter(o => o.trim() !== "");
            opportunities.forEach(op => this.allOpportunities.push(op));
            analysis.contato = websiteData.title || `Time da ${storeName}`;
            ...
            if (!websiteData.hasInstagram) {
                ...
                analysis.oportunidade += ` | Baixo número de seguidores (${websiteData.instagramFollowers}) - potencial para crescimento orgânico`;
            }
            
            if (websiteData.loadingSpeed > 3.0) {
                analysis.oportunidade += ` | Site lento (${websiteData.loadingSpeed.toFixed(2)}s) - otimização de performance pode melhorar SEO e experiência do usuário`;
            } else if (websiteData.loadingSpeed > 1.5) {
                analysis.oportunidade += ` | Velocidade de carregamento moderada (${websiteData.loadingSpeed.toFixed(2)}s) - pequenas otimizações podem fazer diferença`;
            }
            
            if (!websiteData.isMobileFriendly) {
                analysis.oportunidade += ' | Site não otimizado para mobile - perdendo tráfego e vendas de usuários mobile';
            }
            
            if (!websiteData.hasContactForm) {
                analysis.oportunidade += ' | Ausência de formulário de contato - dificultando a comunicação direta com clientes';
            }

            if (websiteData.productCount < 50) {
                analysis.oportunidade += ` | Baixo número de produtos (${websiteData.productCount}) - expansão do catálogo pode atrair mais clientes`;
            }

            if (websiteData.whatsapp === 'Não encontrado') {
                analysis.oportunidade += ' | WhatsApp não encontrado - perdendo canal de atendimento rápido e direto';
            }
            ...
            if (websiteData.hasBlog) {
                ...
                if (monthsDiff > 6) {
                    analysis.oportunidade += ' | Blog abandonado há mais de 6 meses - conteúdo desatualizado pode prejudicar autoridade e SEO';
                } else {
                    analysis.conquista += ' | Blog ativo com conteúdo relevante';
                }
            } else {
                analysis.oportunidade += ' | Sem blog para gerar autoridade e tráfego orgânico - grande potencial para marketing de conteúdo';
            }
        }
